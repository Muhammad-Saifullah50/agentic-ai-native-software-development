...
export const editWorkflow = async (simulationId: string, command: string): Promise<SimulateResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/simulations/${simulationId}/edit`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
